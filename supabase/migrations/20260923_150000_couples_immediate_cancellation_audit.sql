-- Clarify audit action for immediate joint cancellations. No data changes; historical events are preserved.
-- Safely replaces the same super-admin-only, atomic couples cancellation RPC.
...
begin
  ...
  loop
    ...
    insert into public.bgm_audit_log (
      ...
      case when p_action = 'cancel' and p_effective_date = v_today then 'member.couples_cancellation.immediate'
        when p_action = 'cancel' then 'member.couples_cancellation.schedule'
