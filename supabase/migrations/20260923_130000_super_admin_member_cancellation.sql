-- Cancellation is separate from expiry, paid applications and immutable visit history.
-- The effective date is interpreted at Malta midnight by ALL access paths.
-- Only currently unshared, unambiguous individual memberships or legacy-only members
-- can be cancelled here. Joint couples cancellation requires another approved action.
alter table public.bgm_members
  ...
  add column if not exists cancellation_recorded_by uuid references public.bgm_system_users(id) on delete set null;
...
begin
  ...
  insert into public.bgm_audit_log (
    ...
    case when p_action = 'cancel' then 'member.membership_cancellation.schedule'
