...
const gymStaffAccess = [
  ...
  "Member photo view and capture",
  "Reception / barcode scanning",
  "Assign and replace preprinted member cards",
  "Submit sundries orders",
  "Submit bar lists",
];
...
function GymStaffAccessSummary() {
  return (
    <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-4">
      ...
      <p className="mt-1 text-xs leading-5 text-zinc-500">Fixed operational role. Individual permission checkboxes are not used.</p>
...
export default function SystemUsersAdmin() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!usersResponse.ok) {
        throw new Error(usersResponse.status === 401 ? "Please authenticate in the main BGM Admin page first." : "Could not load system users.");
      }
  ...
  async function createUser(event: React.FormEvent) {
    ...
    const response = await fetch("/api/admin/system-users", {
      ...
      body: JSON.stringify({ gymId: isSuperAdmin ? null : gymId, username, displayName, password, isSuperAdmin }),
    });
    const data = await response.json();
    if (!response.ok) { setError(data.error || "Could not create system user."); return; }
    setMessage(`${data.user.displayName} created.`);
    setUsername(""); setDisplayName(""); setPassword(""); setGymId("");
    await load();
  }
  ...
  async function updateUser(user: SystemUser, changes: Record<string, unknown>) {
    ...
    const data = await response.json();
    if (!response.ok) { setError(data.error || "Could not update system user."); return; }
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-zinc-600">One shared Gym Staff login per gym. Super Admin accounts are individual and unrestricted.</p>
          </div>
        ...
        <form onSubmit={createUser} className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 flex gap-3">
            <button type="button" onClick={() => setIsSuperAdmin(false)} className={`rounded-xl px-4 py-2 text-sm font-semibold ${!isSuperAdmin ? "bg-orange-500 text-white" : "bg-zinc-100"}`}>Gym Staff</button>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            {!isSuperAdmin && <label className="text-sm font-medium">Gym<select required value={gymId} onChange={(e) => setGymId(e.target.value)} className="mt-1 w-full rounded-xl border border-zinc-300 px-3 py-2"><option value="">Select gym</option>{availableGyms.map((gym) => <option key={gym.id} value={gym.id}>{gym.name}</option>)}</select></label>}
            <label className="text-sm font-medium">Display name<input required value={displayName} onChange={(e) => setDisplayName(e.target.value)} className="mt-1 w-full rounded-xl border border-zinc-300 px-3 py-2" placeholder={isSuperAdmin ? "Super Admin" : "Birkirkara Fitness"} /></label>
            <label className="text-sm font-medium">Username<input required value={username} onChange={(e) => setUsername(e.target.value)} className="mt-1 w-full rounded-xl border border-zinc-300 px-3 py-2" placeholder={isSuperAdmin ? "superadmin" : "birkirkarafitness"} /></label>
            <label className="text-sm font-medium">Password<input required minLength={8} type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="mt-1 w-full rounded-xl border border-zinc-300 px-3 py-2" placeholder="At least 8 characters" /></label>
          </div>
          {!isSuperAdmin && <div className="mt-5"><GymStaffAccessSummary /></div>}
          {isSuperAdmin && <div className="mt-5 rounded-xl border border-orange-200 bg-orange-50 p-4 text-sm font-semibold text-orange-800">Super Admin has unrestricted access to all BGM operations and management tools.</div>}
        ...
        <section className="space-y-4">
          ...
          {users.length === 0 && <div className="rounded-2xl border border-zinc-200 bg-white p-5 text-sm text-zinc-600">No system users yet.</div>}
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2"><h3 className="font-bold">{user.displayName}</h3><span className={`rounded-full px-2 py-1 text-xs font-bold ${user.isSuperAdmin ? "bg-orange-100 text-orange-700" : "bg-zinc-100 text-zinc-700"}`}>{user.isSuperAdmin ? "SUPER ADMIN" : "GYM STAFF"}</span></div>
      ...
      {!user.isSuperAdmin && <div className="mt-4"><GymStaffAccessSummary /></div>}
