...
import type {
  IdentityMatchState,
  PublicEnrollmentConfig as CanonicalPublicEnrollmentConfig,
  RegistrationDraft as CanonicalRegistrationDraft,
  RegistrationParticipant,
} from "@/lib/membershipRegistrationTypes";
import type { MembershipDurationKey, MembershipType } from "@/lib/membershipSettingsCore";
import StaffRegistrationPhotoControls from "@/components/staff/StaffRegistrationPhotoControls";
...
export type PublicEnrollmentConfig = CanonicalPublicEnrollmentConfig;
export type RegistrationDraft = CanonicalRegistrationDraft & { gymSlug: string };
...
function formatEur(amountCents: number) {
  return new Intl.NumberFormat("en-MT", { style: "currency", currency: "EUR" }).format(amountCents / 100);
}
...
export default function RegistrationForm({
  ...
  const [declarations, setDeclarations] = useState<RegistrationDeclarationAcceptance[]>([{ ...BLANK_ACCEPTANCE }]);
  ...
  function updateGuardian(index: number, field: GuardianField, value: string) {
    setParticipants((current) => current.map((participant, participantIndex) => {
      ...
      const guardian = participant.guardian || { fullName: "", idNumber: "", relationship: "", mobile: "", email: "", address: "" };
  ...
  function updateAcceptance(index: number, value: RegistrationDeclarationAcceptance) {
    setDeclarations((current) => current.map((acceptance, participantIndex) => participantIndex === index ? value : acceptance));
  }

  function updatePhoto(index: number, file: File) {
    setPhotos((current) => current.map((photo, participantIndex) => participantIndex === index ? file : photo));
    setFormError("");
  }

  function clearPhoto(index: number) {
    setPhotos((current) => current.map((photo, participantIndex) => participantIndex === index ? null : photo));
    setFormError("");
  }
  ...
  async function checkIdentity(index: number) {
    ...
    setIdentityStates((current) => current.map((state, participantIndex) => participantIndex === index ? "checking" : state));
    setIdentityMessages((current) => current.map((message, participantIndex) => participantIndex === index ? "Checking existing memberships…" : message));
    ...
    try {
      ...
      if (!response.ok || !["clear", "active", "expired_inactive"].includes(data.state)) throw new Error("Identity check failed");
      const state = data.state as IdentityMatchState;
      setIdentityStates((current) => current.map((item, participantIndex) => participantIndex === index ? state : item));
      setIdentityMessages((current) => current.map((message, participantIndex) => {
        if (participantIndex !== index) return message;
        if (state === "active") return "An active membership already exists for this ID. Please speak to reception.";
        if (state === "expired_inactive") return "An existing membership was found. Reception will review this application as a possible renewal.";
        return "";
      }));
    } catch {
      setIdentityStates((current) => current.map((state, participantIndex) => participantIndex === index ? "error" : state));
      setIdentityMessages((current) => current.map((message, participantIndex) => participantIndex === index ? "We could not check this ID right now. Please try again before submitting." : message));
    }
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    for (let index = 0; index < enrollmentParticipants.length; index += 1) {
      ...
      if (mode === "tablet" && (identityStates[index] === "checking" || identityStates[index] === "error")) {
        setFormError("Complete the ID check before submitting.");
        return;
      }
      if (!declarations[index]?.gymRules || !declarations[index]?.privacy || !declarations[index]?.health) {
  ...
  return (
    <form onSubmit={submit} className="space-y-5" noValidate>
      <section className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-2 text-sm leading-6 text-zinc-600">Choose the membership that applies to you. Reception will verify the supporting documents before activation.</p>
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          {MEMBERSHIP_OPTIONS.map((option) => (
            <button key={option.value} type="button" onClick={() => chooseMembershipType(option.value)} className={`rounded-2xl border px-4 py-4 text-left font-black transition ${membershipType === option.value ? "border-zinc-950 bg-zinc-950 text-white" : "border-zinc-200 bg-zinc-50 text-zinc-950 hover:border-zinc-400"}`}>
              {option.label}
            </button>
          ))}
        </div>
      ...
      {membershipType && documentReady && participants.map((participant, index) => {
        ...
        return (
          <section key={index} className="space-y-5 rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.18em] text-zinc-500">{membershipType === "couples" ? `Couples applicant ${index + 1}` : "Applicant details"}</p>
              <h2 className="mt-1 text-xl font-black text-zinc-950">{membershipType === "couples" ? `Applicant ${index + 1}` : "Your details"}</h2>
            </div>
            ...
            {under18 && membershipType !== "couples" ? (
              <div className="rounded-3xl border border-violet-200 bg-violet-50 p-5">
                ...
                <p className="mt-1 text-sm leading-6 text-violet-900">The guardian must attend reception and co-sign the printed application before this membership can be activated.</p>
                <div className="mt-4 grid gap-4 sm:grid-cols-2">
                  <label className="text-sm font-bold text-violet-950">Guardian full name<input value={participant.guardian?.fullName || ""} onChange={(event) => updateGuardian(index, "fullName", event.target.value)} className={inputClass()} /></label>
                  <label className="text-sm font-bold text-violet-950">Guardian ID / passport number<input value={participant.guardian?.idNumber || ""} onChange={(event) => updateGuardian(index, "idNumber", event.target.value)} className={inputClass()} /></label>
                  <label className="text-sm font-bold text-violet-950">Relationship<input value={participant.guardian?.relationship || ""} onChange={(event) => updateGuardian(index, "relationship", event.target.value)} className={inputClass()} /></label>
                  <label className="text-sm font-bold text-violet-950">Guardian mobile<input type="tel" value={participant.guardian?.mobile || ""} onChange={(event) => updateGuardian(index, "mobile", event.target.value)} className={inputClass()} /></label>
                  <label className="text-sm font-bold text-violet-950">Guardian email<input type="email" value={participant.guardian?.email || ""} onChange={(event) => updateGuardian(index, "email", event.target.value)} className={inputClass()} /></label>
                  <label className="text-sm font-bold text-violet-950">Guardian address<input value={participant.guardian?.address || ""} onChange={(event) => updateGuardian(index, "address", event.target.value)} className={inputClass()} /></label>
                </div>
              </div>
            ) : null}

            {mode === "tablet" ? (
              <LivePhotoCapture label={`Applicant ${index + 1} live membership photo`} required={true} onUsePhoto={(file) => updatePhoto(index, file)} />
            ) : null}
            {mode === "staff" ? (
              <StaffRegistrationPhotoControls
                onCaptured={(file) => updatePhoto(index, file)}
                onUploaded={(file) => updatePhoto(index, file)}
                onPhotoLater={() => clearPhoto(index)}
              />
            ) : null}
            ...
            <RegistrationDeclarations
              ...
              showGuardian={under18 && membershipType !== "couples"}
              showUnder16Supervision={under16 && membershipType !== "couples"}
      ...
      {formError ? <div role="alert" className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm font-bold text-red-800">{formError}</div> : null}
      {membershipType && documentReady ? (
        <button type="submit" disabled={submitting || !durationKey} className="w-full rounded-2xl bg-zinc-950 px-5 py-4 text-base font-black text-white shadow-lg disabled:cursor-not-allowed disabled:opacity-50">
