...
function StaffNewMembershipEnrollment({ memberNumber }: { memberNumber: string }) {
  ...
  const [success, setSuccess] = useState<{ reference: string; photoWarnings: string[] } | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        setError("");
        const response = await fetch("/api/system/members/enroll", { method: "GET", cache: "no-store" });
        const payload = await response.json().catch(() => ({}));
        if (!response.ok) throw new Error(payload?.error || "Could not load membership registration settings.");
        if (!active) return;
  ...
  async function uploadParticipantPhoto(applicationMemberId: string, file: File, participant: RegistrationDraft["participants"][number], index: number) {
    ...
    const response = await fetch("/api/system/members/photo", { method: "POST", body: formData });
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(payload?.error || `Could not save photo for participant ${index + 1}.`);
  }
    ...
    try {
      ...
      for (let index = 0; index < draft.participants.length; index += 1) {
        ...
        try {
          ...
          photoWarnings.push(photoError instanceof Error ? photoError.message : `Participant ${index + 1}: application was saved, but the photo could not be saved.`);
        }
      }
  ...
  if (loading) return <div className="mx-auto max-w-5xl p-6 text-white"><p className="text-sm text-white/65">Loading membership registration…</p></div>;
  if (!config) return <div className="mx-auto max-w-5xl p-6 text-white"><h1 className="text-2xl font-semibold">New Membership</h1><p className="mt-3 rounded-2xl border border-red-400/25 bg-red-500/10 p-4 text-sm text-red-100">{error || "Membership registration settings are not available."}</p></div>;
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-5 p-4 pb-28 text-white sm:p-6">
      <div className="rounded-3xl border border-white/10 bg-white/[0.055] p-5 shadow-2xl backdrop-blur-xl">
        ...
        <p className="mt-2 text-sm text-white/60">{config.gym.name} · Complete the same registration details used on the member tablet. Photos can be taken, uploaded, or added later.</p>
        {memberNumber ? <p className="mt-3 text-xs text-amber-200">Member lookup reference: {memberNumber}</p> : null}
      </div>
      {error ? <div className="rounded-2xl border border-red-400/25 bg-red-500/10 p-4 text-sm text-red-100">{error}</div> : null}
      {success ? <div className="rounded-2xl border border-emerald-400/25 bg-emerald-500/10 p-4 text-sm text-emerald-100"><p className="font-semibold">Application {success.reference} saved for staff review.</p>{success.photoWarnings.length ? <div className="mt-2 space-y-1 text-amber-100">{success.photoWarnings.map((warning) => <p key={warning}>{warning}</p>)}<p>Missing photos can be added later and do not cancel the application.</p></div> : null}</div> : null}
      <RegistrationForm mode="staff" config={config} submitting={submitting} submitLabel="Save New Membership" staffPhotoPolicy="optional" onSubmit={submitRegistration} />
    </div>
  );
}
